...
Each enabled check gets one sensor. Its state is how many of that check's Repairs cards are open, not counting cards you ignored; it is recounted after each run. The size of every group is in its `counts` attribute.
