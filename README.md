...
`sensor.gut_check_home_health_check` shows the number of entities it sorted confidently (expected plus worth fixing plus safe to remove). Its attributes carry the full list for each group, plus an `unsure` list for anything below the confidence threshold or that did not clearly fit any group. Unsure entities are never counted in the sensor's number, never turned into a Repairs card, and never treated as worth fixing. An unsure entity may also carry a `lean` of `needs_attention` or `expected`, when the model leans clearly one way without being sure enough of the group to place it there; a lean never raises a Repairs card.
