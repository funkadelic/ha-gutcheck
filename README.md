...
An area suggestion gets a Repairs card the same way, one per device, up to ten new cards per run, most confident first; the rest wait for a later run or a button press. A card clears when you confirm it, or when the device gets an area another way or stops qualifying. An ignored card stays ignored for as long as the device still qualifies, including across turning area suggestions off and back on. If the device or the suggested area changed by the time you submit a card, confirming it does nothing and tells you so.
