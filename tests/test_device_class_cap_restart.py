...
from custom_components.gutcheck.const import DEVICE_CLASS_ISSUE_PREFIX, DOMAIN, MAX_NEW_DEVICE_CLASS_CARDS_PER_RUN
...
async def test_a_restart_raises_no_card_the_run_held_back_over_the_cap(
    ...
    assert len(state.attributes["items"]["suggested"]) > MAX_NEW_DEVICE_CLASS_CARDS_PER_RUN
    assert state.state == str(MAX_NEW_DEVICE_CLASS_CARDS_PER_RUN)
    run_cards = _card_ids(hass)
    assert len(run_cards) == MAX_NEW_DEVICE_CLASS_CARDS_PER_RUN
